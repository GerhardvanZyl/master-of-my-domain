...
export default function PropertyGrid({
  ...
  // Hydrate the saved vibe config after mount. Reading localStorage DURING a
  // render makes the client markup disagree with the server's and React throws
  // a hydration error, so the whole config lives in state.
  const [savedCfg, setSavedCfg] = useState(DEFAULT_VIBE_CONFIG);
  useEffect(() => {
    const c = loadVibeConfig();
    setSavedCfg(c);
    setIdealPrice(c.idealPrice);
  }, []);
  ...
  return (
    <>
      ...
      <div className="mb-7 flex flex-wrap items-center gap-x-5 gap-y-3.5 rounded-2xl border border-line bg-paper p-4 shadow-[0_1px_2px_rgba(0,0,0,.03)]">
