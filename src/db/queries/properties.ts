...
// pickHero/isVisibleImage moved to src/lib/photo.ts (arch-003): they are pure
// image-selection policy with no DB access, and this module imports `db` at
// load time, which opens/migrates data/app.db as a side effect (see
// src/db/client.ts) — merely importing pickHero for that policy paid for a
// live DB connection. That side effect is still live and measured: importing
// this module with DB_PATH/DATA_DIR redirected creates a ~151 KB database
// plus WAL/SHM sidecars and an images/ directory. The re-export below is a
// COMPATIBILITY SHIM for existing callers only — new code, and anything under
// scripts/, must import these from @/lib/photo instead, to avoid paying for
// that connection.
import { aspect, isPropertyPhoto, isHeroPhoto, isVisibleImage, pickHero } from "@/lib/photo";
export { isPropertyPhoto, isVisibleImage, pickHero };
