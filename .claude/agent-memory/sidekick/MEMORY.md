...
- [SQLite transaction atomicity test via trigger](sqlite_transaction_atomicity_test_via_trigger.md) — force a mid-transaction failure with a scoped TEMP TRIGGER + RAISE(ABORT), no production seam needed; confirmed on deleteProperty's scrape_jobs detach
- [Path traversal guard limits + pre-check gating](path_traversal_guard_limits_and_precheck_gating.md) — dirname/basename guard replaced by an fs.readdirSync(root).includes(id) identity check (Windows aliases); ambiguous listingUrls ref resolution algorithm; fs.rmSync monkeypatch technique
